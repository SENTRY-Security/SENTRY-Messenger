  ...
  async function maybeAutogenerateAvatar() {
    ...
    try {
      ...
      const next = {
        ...
        nickname: state?.nickname || generateRandomNickname(),
        updatedAt: Math.floor(Date.now() / 1000)
      };
      const saved = await saveProfile(next);
      sessionStore.profileState = {
        ...(saved || next),
        avatar: { ...(saved?.avatar || avatarMeta), autoGenerated: true }
      };
      log({ profileAvatarAutogen: avatarMeta.objKey });
    } catch (err) {
      log({ profileAvatarAutogenError: err?.message || err });
    }
  }
