...
const subtle: SubtleCrypto = cryptoObj.subtle;
...
interface NormalizeInfoTagOptions {
  allowInfoTags?: Set<string> | null;
  required?: boolean;
}

interface AssertEnvelopeOptions {
  requireInfoTag?: boolean;
  allowInfoTags?: Set<string>;
}

interface EncryptParams {
  key: CryptoKey;
  iv: Uint8Array | ArrayBufferView;
...
interface DecryptParams {
  key: CryptoKey;
  iv: Uint8Array | ArrayBufferView;
  ciphertext: Uint8Array | ArrayBufferView;
}
...
export const ALLOWED_ENVELOPE_INFO_TAGS: ReadonlySet<string> = new Set([
  'blob/v1',
  'media/v1',
  'profile/v1',
  'settings/v1',
...
function normalizeInfoTag(
  infoTag: unknown,
  { allowInfoTags = null, required = true }: NormalizeInfoTagOptions = {},
): string | null {
...
export function assertEnvelopeStrict(
  envelope: Record<string, unknown>,
  { requireInfoTag = true, allowInfoTags = ALLOWED_ENVELOPE_INFO_TAGS as Set<string> }: AssertEnvelopeOptions = {},
): AeadEnvelope {
  ...
  const info = requireInfoTag
    ...
    : (typeof envelope.info === 'string' ? (envelope.info as string).trim() : (envelope.info as string));
  return {
    ...envelope,
    aead: 'aes-256-gcm',
    info,
...
async function deriveAesKey(
  ...
  const mkKey = await subtle.importKey(
    'raw',
    toU8Strict(mkRawU8, 'web/src/shared/crypto/aead.ts:deriveAesKey') as BufferSource,
    'HKDF',
    false,
    ['deriveKey'],
  );
  ...
  return subtle.deriveKey(
    { name: 'HKDF', hash: 'SHA-256', salt: saltU8 as BufferSource, info },
    mkKey,
...
export async function encryptAesGcm({ key, iv, data }: EncryptParams): Promise<EncryptResult> {
  if (!(key instanceof CryptoKey)) throw new Error('encryptAesGcm: key required');
  const buf = data instanceof Uint8Array ? data : encoder.encode(data as string);
  const ivBytes = iv instanceof Uint8Array ? iv : toUint8Array(iv);
  const ct = new Uint8Array(await subtle.encrypt({ name: 'AES-GCM', iv: ivBytes as BufferSource }, key, buf as BufferSource));
  return { iv: ivBytes, ciphertext: ct };
}

export async function decryptAesGcm({ key, iv, ciphertext }: DecryptParams): Promise<Uint8Array> {
  ...
  const pt = await subtle.decrypt({ name: 'AES-GCM', iv: ivBytes as BufferSource }, key, ctBytes as BufferSource);
...
export function encodeUtf8(value: Uint8Array | string | null | undefined): Uint8Array {
  return value instanceof Uint8Array ? value : encoder.encode(String(value ?? ''));
}

export function decodeUtf8(u8: Uint8Array | ArrayBufferLike): string {
  return decoder.decode(u8 instanceof Uint8Array ? u8 : new Uint8Array(u8));
}

export function toUint8Array(value: Uint8Array | string | ArrayBufferView | ArrayBuffer): Uint8Array {
...
export async function wrapWithMK_JSON(
  obj: unknown,
  mkRawU8: Uint8Array,
  infoTag: string = 'blob/v1',
...
export async function unwrapWithMK_JSON(
  envelope: Record<string, unknown>,
  mkRawU8: Uint8Array,
): Promise<unknown> {
