...
- **目前狀態**：媒體簽章 API 已強制攜帶帳號憑證並通過會話 ACL 驗證，`npm run test:{prekeys-devkeys,messages-secure,friends-messages,login-flow,front:login}` 全數綠燈。
- **下一步**：觀察舊帳號是否缺少 conversation ACL，必要時補做批次廣播或後端回填。


| 日期                    | 里程碑                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **2025-11-09（Codex）** | `/media/sign-put|get` 強制驗證登入帳號並透過 Worker `/d1/conversations/authorize` 檢查會話授權；好友聯絡同步會在 Worker 登記 `conversation_acl` 以授權日後簽名；前端簽名請求自動帶入憑證，`scripts/r2-media-test.sh` 更新支援新欄位。`npm run test:{prekeys-devkeys,messages-secure,friends-messages,login-flow,front:login}` 全數通過。                                                                                                                                                                                                                                                                                                                    |
