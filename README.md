...
- **目前狀態**：系統設定頁完成「當畫面不在前台時自動登出」子選項，支援預設 / 客製化登出頁面單選；客製化改為以獨立 Modal 管理網址（僅在那裡輸入 HTTPS），主設定視窗不再出現捲軸。`settings-<acctDigest>` 會帶入 `autoLogoutRedirectMode`＋`autoLogoutCustomUrl` 並持續以 MK-AEAD 加密，`secureLogout()` 依設定導向預設或自訂頁面。本輪也完成 D1 / R2 清空與 Worker / Node API / Pages 重新部署。
