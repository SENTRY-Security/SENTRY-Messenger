...
- **目前狀態**：Playwright e2e 以 WebKit（iPhone 13 Pro profile）為預設執行，`tests/e2e/call-audio.spec.mjs` 改由獨立的 Chromium project 搭配 fake audio 裝置跑通；full-flow 測試允許 WebKit 缺少 `MediaRecorder` 時跳過影片附件並延長 timeout。`npm run test:{prekeys-devkeys,messages-secure,friends-messages,login-flow}` 維持綠燈，但 `npm run test:front:login` 在 WebKit 上仍因 `tests/e2e/full-flow.spec.mjs` 最後驗證附件截圖時超時（長流程 + Safari cascade）而失敗，後續需針對該段縮圖檢查加速或拆測。
- **下一步**：針對 full-flow WebKit case 重新設計附件驗證（例如改為直接觸發預覽、收斂流程步驟或拆分測試），讓 `npm run test:front:login` 能在 WebKit 內合理時間內完成；同時持續觀察 Production D1 prekeys/devkeys 補發狀況。
