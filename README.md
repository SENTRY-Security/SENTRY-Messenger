...
- [ ] WebSocket 流：`src/ws/index.js` 以 UID 為連線鍵（call locks/presence/secure-message event）且 payload 帶 `peerUid`，`web/src/app/api/ws.js` 要求 `uidHex`；需改為 accountDigest 為主的身份與事件欄位。**進度：WS server 現已移除 UID 映射並以 accountDigest 追蹤連線/鎖定/presence；前端通話信令與 presence payload 改用 accountDigest（移除 `targetUid`）；仍有歷史變數名與其他模組 payload 待清。**
