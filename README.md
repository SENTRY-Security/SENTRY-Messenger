...
- **目前狀態**：系統設定頁完成「當畫面不在前台時自動登出」子選項，支援預設 / 客製化登出頁面單選、可編輯下拉（Google/Apple 等）與 HTTPS 驗證；儲存後 `settings-<acctDigest>` 會帶入 `autoLogoutRedirectMode`＋`autoLogoutCustomUrl` 並持續以 MK-AEAD 加密，`secureLogout()` 也會依設定導向預設頁或自訂網址。本輪也完成 D1 / R2 清空與 Worker / Node API / Pages 重新部署。
