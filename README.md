...
- **目前狀態**：通話媒體層已修正 WebSocket 信令（offer/answer/candidate）與 ICE 佇列，確保 callee 會正確回傳 `call-answer` 並在 remoteDescription 建立前緩存候選；Playwright e2e 現在會強制通話至少維持 3 秒並檢測雙向音訊振幅。登入 App 後會跳出「啟用語音／影像通話」提示，藉由 `getUserMedia` 預先取得麥克風 / 鏡頭權限並解鎖音訊播放，避免 iOS 裝置再度無聲；Node API / Worker / Pages 皆已重新部署。NAT / TURN 組態改由 `/api/v1/calls/network-config` 依環境變數即時輸出並於前端自動合併 STUN + TURN 憑證，弱網行為可透過設定調整而不需重新打包 Pages。
- **下一步**：延伸同組檢查到視訊 / 多裝置情境，並觀察 call overlay／remote console 在 QA regression 中的行為；完成 TURN 服務實體佈署後，需在 5G ↔︎ Wi-Fi 等跨 NAT 情境重新驗證 `tests/e2e/call-audio`。
